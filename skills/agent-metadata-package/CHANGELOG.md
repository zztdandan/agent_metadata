...
1. **README 重写**：README 不再把本仓库描述为元数据包，改为准确描述本仓库的两种角色——规范来源（`docs/`）和编制技能来源（`skills/agent-metadata-package/`）。删除不存在的根级 `package-metadata.json`、`schema/`、`common/`、`capabilities/` 等目录展示；删除与编制技能和 BOOTSTRAP 职责重复的"给自举智能体的最短路径"和"给维护者的路径"段。新增"制作包"和"使用包"两种工作流说明。
   - 迁移动作：无（文档修正，不影响元数据包结构）。

2. **新增 LICENSE 文件**：本项目采用 MIT 许可证。README 许可证段从"暂未附带 LICENSE 文件"改为声明 MIT，并明确智能体元数据包作为独立产物可自行定义许可证。
   - 迁移动作：无（项目许可证，不影响元数据包结构）。

3. **README 改编为 GitHub 项目形态**：开篇增加 shields 徽章与大写项目名，并以“搬家专家”说明动机——好的 harness 不断更替导致工作流要反复迁移，而办公智能体（如 WorkBuddy）把专家和技能锁在平台内、迁不出去；`agent_metadata` 提供智能体无关的打包模式，把软装带走后在目标宿主按布局重新布置（自举），并可把能力域以运行时无关的形式分享给他人。正文改为“如何打包”（加载编制技能）与“如何自举”（目标智能体直接接收包归档，或直接接收 `BOOTSTRAP.md`）两条入门路径，并补充“如何阅读本项目”的角色化阅读路径。`docs/` 与编制技能内容未变。
   - 迁移动作：无（文档修正，不影响元数据包结构）。
