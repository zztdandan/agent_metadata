...
每次 `schemaVersion` 升级、字段增删、目录结构变化或规则约束变化时，**必须**在根目录 `CHANGELOG.md` 追加一条记录，包含：变更编号、变更类型（破坏性/语义收窄/文档修正）、变更描述和迁移动作。用户从旧版本迁移到新版本时，应对照 `CHANGELOG.md` 逐项检查。

`CHANGELOG.md` 只记录规约规则的变更，不记录 Git commit、包内容修订或资产增减。
